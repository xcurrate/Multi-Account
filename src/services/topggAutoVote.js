...
const getTargetUrls = (topgg = {}) => [topgg.topggUrl, topgg.topggUrl2, topgg.topggUrl3]
...
const normalizeTopggConfig = (config = {}) => {
    ...
    topgg.discordToken = String(config.token || '');
    topgg.targetUrls = getTargetUrls(topgg);
    return topgg;
};

const isCompletedTargetStatus = (status) => status === STATUSES.SUCCESS || status === STATUSES.ALREADY_VOTED;
...
const createTopggAutoVoteService = (configManager) => {
    ...
    const runOnce = async () => {
        ...
        try {
            ...
            if (!topggConfig.discordToken) {
                logger.error('Token akun aktif kosong; Auto Vote Top.gg dibatalkan');
                lastStatus = STATUSES.FAILED;
                return lastStatus;
            }

            if (topggConfig.targetUrls.length === 0) {
                logger.error('Belum ada target vote Top.gg yang dikonfigurasi');
                lastStatus = STATUSES.FAILED;
                return lastStatus;
            }

            logger.info(`Menggunakan token akun aktif untuk ${topggConfig.targetUrls.length} target vote`);
            ...
            let hasSuccessfulVote = false;
            let hasAlreadyVoted = false;

            for (let index = 0; index < topggConfig.targetUrls.length; index += 1) {
                const targetUrl = topggConfig.targetUrls[index];
                logger.info(`Menjalankan antrean vote ${index + 1}/${topggConfig.targetUrls.length}: ${targetUrl}`);

                const targetStatus = await performVote(page, { ...topggConfig, topggUrl: targetUrl }, logger);
                logger.info(`Status target vote ${index + 1}/${topggConfig.targetUrls.length}: ${targetStatus}`);

                if (!isCompletedTargetStatus(targetStatus)) {
                    lastStatus = targetStatus;
                    logger.warn(`Antrean vote dihentikan karena target ${index + 1} belum sukses`);
                    return lastStatus;
                }

                hasSuccessfulVote = hasSuccessfulVote || targetStatus === STATUSES.SUCCESS;
                hasAlreadyVoted = hasAlreadyVoted || targetStatus === STATUSES.ALREADY_VOTED;

                if (index < topggConfig.targetUrls.length - 1) {
                    logger.info(`Target ${index + 1} selesai; melanjutkan ke target berikutnya`);
                }
            }

            lastStatus = hasSuccessfulVote ? STATUSES.SUCCESS : (hasAlreadyVoted ? STATUSES.ALREADY_VOTED : STATUSES.FAILED);
            logger.info(`Status akhir antrean Auto Vote Top.gg: ${lastStatus}`);
            return lastStatus;
        } catch (error) {
...
module.exports.getTargetUrls = getTargetUrls;
